#'@title creates a dataframe containing the points for the deltoide mandala
#'@name pdeltoide
#'@author Luciane Ferreira Alcoforado
#
#'
#'@description  Function to build a deltoide
#'@param theta is a vector length 2 with start angle and end angle
#'@param raio is a vector length 1 with radius value. For deltoide we do r=2.
...
pdeltoide = function(theta, raio, k, n){
theta = seq(theta[1],theta[2],length.out=n)
raio=2
x=raio*cos(theta)+cos(raio*theta)
y=raio*sin(theta)-sin(raio*theta)
